...
const ExpenseAprove = () => {
  ...
  const handleClickForOneUser = async (userID) => {
    ...
    try {
      const { data } = await axios.get(
        `http://localhost:8000/api/user-submited-expense/?userID=${userID}`,
        config
      );
      console.log("this user Timesheet", data.allExpenseData);
      setTableData(data.allExpenseData);
    } catch (error) {
  ...
  useEffect(() => {
    const getAllSubmitedExpense = async () => {
      ...
      try {
        const { data } = await axios.get(
          `http://localhost:8000/api/all-submited-expense/?companyID=${companyID}`,
          config
        );
        setAllsubmitedExpense(data.allSubmitedExpenseData);
      } catch (error) {
  ...
  const TimesheetAprovedNotfy = () => {
    toast.success('Expense has been aproved', {
      position: 'top-right',
      autoClose: 6000,
              ...
              <Paper style={aproveFilterBy == "byEmployee" ? { display: "block" } : { display: "none" }} sx={{ width: "100%", overflow: "hidden" }}>
                ...
                >
                  <AproveTimesheetTable
                    columns={columnsForExpenseTable}
                    userExpenseInfo={tableData}
              ...
              <Paper style={aproveFilterBy == "byEmployee" ? { display: "none" } : { display: "block" }} sx={{ width: "100%", overflow: "hidden" }}>
                <TableContainer sx={{ maxHeight: 360 }}>
                  ...
                  >
                    <TableHead>
                      <TableRow>
                        {columnsForAllSubmitedExpenseTable.map((column) => (
                          <TableCell
                            key={column.id}
                            align={column.align}
                            style={{
                    ...
                    >
                      {allsubmitedExpense
                        ? allsubmitedExpense?.map((item, index) => {
                          return (
